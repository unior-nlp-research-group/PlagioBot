...
def state_WAITING_FOR_START(user, message_obj):
    ...
    if message_obj is None:        
        ...
        game_name_type = ux.MSG_GAME_NAME[lang].format(game.get_name(), game.game_type)
        ...
        if user == creator:
            ...
            send_message_multi(others_players, msg_others)
        ...
        if text_input:
            if text_input in utility.flatten(kb):
                if text_input.startswith(ux.BUTTON_BACK[lang]):
                    ...
                    send_message_multi(writers, msg)
                    redirect_to_state(user, state_GAME_SETTINGS)
            elif ux.text_is_button_or_digit(text_input):
                send_message(user, ux.MSG_WRONG_BUTTON_INPUT[lang], kb)
        else:
    ...
    if message_obj is None:
        if user == players[0]:
            if hand_number == 1:
                ...
                send_message_multi(players, ux.MSG_INSTRUCTIONS[game.game_type][lang])
            ...
            msg_intro = '\n'.join(msg_intro_list)
            send_message_multi(players, msg_intro, remove_keyboard=True)
...
def state_WRITERS_WRITE_ANSWERS(user, message_obj):
    ...
    if message_obj is None:
        if user == players[0]:
            msg_reader = ux.MSG_WAIT_WRITERS_WRITE_ANSWERS[game.game_type][lang]
            ...
            if ux.text_is_button_or_digit(text_input):
                ...
                if utility.contains_markdown(answer):
                    ...
                    if len(remaining_names)>0:                        
                        ...
                        msg_list = [
                            ux.RECEIVED_ANSWER_BY[lang].format(user.get_name()),
                            ux.MSG_LETS_WAIT_FOR[lang].format(remaining_names_str),
                        ]
...
def state_WRITERS_VOTE_ANSWER(user, message_obj):
    ...
    players_num_votes = len(shuffled_answers_info) - 1
    ...
    if message_obj is None:
        if user == players[0]:
            ...
            for w in writers:
                ...
                if game.game_control != 'TEACHER' and p_index in correct_author_indexes:
                    ...
                    msg_list = [
                        ux.MSG_GUESSED_NO_VOTE[lang],
                        ux.MSG_WAIT_FOR[lang].format(remaining_names_str)
                    ]
                    send_message(w, '\n'.join(msg_list), remove_keyboard=True)
                else:
        ...
        if user.get_var('NO_VOTE'):
            msg = ux.MSG_WRONG_INPUT_WAIT_FOR_PLAYERS_TO_VOTE[lang]
            send_message(user, msg)
            return
        ...
        if text_input in utility.flatten(kb):
            ...
            remaining_names = game.set_voted_indexes_and_points_and_get_remaining(user, voted_shuffled_number)
            remaining_names_str = ', '.join(remaining_names)
            send_message(user, ux.MSG_THANKS[lang], remove_keyboard=True)
            all_but_users = [p for p in players if p!=user]
...
def state_TEACHER_VALIDATION(user, message_obj):
    ...
    shuffled_answers_info = game.get_shuffled_answers_info(include_no_vote=False)
    number_answers = len(shuffled_answers_info)
    teacher_options = ux.MSG_TEACHER_VOTING_OPTIONS_NONE_ALLOWED[lang] \
        if game.is_voting_no_or_multiple_answers_allowed() \
        else ux.MSG_TEACHER_VOTING_OPTIONS[lang]
    if message_obj is None:
        if user == reader:
            # teacher
            ...
            msg = ux.MSG_TEACHER_VOTE[lang].format(teacher_options)
            ...
            if text_input in utility.flatten(kb):
                ...
                if text_input == ux.BUTTON_SUBMIT[lang]:
                    ...
                    if CORRECT_ANSWERS_NUMBERS:
                        ...
                        msg = ux.MSG_TEACHER_VOTE_AND_SUBMIT[lang].format(teacher_options)
                    else:
...
def recap_votes(user, game):
    ...
    for answer_info in shuffled_answers_info:
        ...
        answer_report += '\n' + ux.MSG_VOTED_BY[lang].format(num_voters_and_names)
        msg_list.append(answer_report)
    send_message_multi(players, '\n\n'.join(msg_list), remove_keyboard=True)
    ...
    for w in writers:
        ...
        msg_points_correct_voting = ux.MSG_POINT_SG_PL(parameters.POINTS['CORRECT_VOTING'])[lang]
        ...
        if p_feedback['ANSWERED_CORRECTLY']:
            ...
            msg_list.append(ux.MSG_WRONG_ANSWER[lang])
        
        if 'VOTED_CORRECTLY' in p_feedback:
            if p_feedback['VOTED_CORRECTLY']:
                msg_list.append(ux.MSG_CORRECT_VOTING[lang].format(msg_points_correct_voting))
            else:
                if game.game_control == 'TEACHER':            
                    msg_points_incorrect_voting = ux.MSG_POINT_SG_PL(parameters.POINTS['INCORRECT_VOTING'])[lang]
                    msg_list.append(ux.MSG_WRONG_VOTING_PENALTY[lang].format(msg_points_incorrect_voting))
                else:
                    msg_list.append(ux.MSG_WRONG_VOTING[lang])
        ...
        if game.game_control != 'TEACHER':
            ...
            msg_list.append(ux.MSG_RECEIVED_VOTED[lang].format(msg_received_votes_total_points))
        
        send_message(w, '\n'.join(msg_list))

    send_message_multi(players, ux.MSG_POINT_ROUND_SUMMARY[lang])            
...
def interrupt_game(game, user=None):
    ...
    if len(players) > 0:
        if user:
            send_message_multi(players, ux.MSG_EXIT_GAME[lang].format(user.get_name()), remove_keyboard=True)
        else:
            send_message_multi(players, ux.MSG_EXIT_GAME_EXPIRED[lang], remove_keyboard=True)
    restart_multi(players)
